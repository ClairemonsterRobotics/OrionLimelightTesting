...
public class armSub extends Subsystem {
    ...
    public boolean moveArmToPosition(double armPos) {
        ...
        if (goingUp == true) {
            ...
            SmartDashboard.putNumber("currentArmPos", currentArmPos);
            if (currentArmPos < armPos) {
                armMotor.set(.4);
    ...
    public int getYDirection(){
        ...
        SmartDashboard.putNumber("y hand val", yhand);
        if (yhand < -0.1){
            armDirection = 1;
        }
        else if (yhand > 0.1){
    ...
    public void moveArmManual(int direction, double sensitivity){
        if (direction == 1){
            armMotor.set(0.4); //adjust the dividing values so that the slider doesnt impact the sensitivity TOO much
        }
        else if (direction == 2){
            armMotor.set(-0.4); // ^^, also sensitivity may need to be inverted
        }
        else if (direction == 0){
            armMotor.set(0);
        }
